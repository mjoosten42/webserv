...
TEST_CASE( "Hosts", "[Server]" ) {
	...
	REQUIRE( test == "html/img"); //Check loc 0 for file it contains

	loc_index = s.getLocationIndexForFile("amogus_bogus_file");
	REQUIRE( loc_index == 1);
	test = s.getRootForFile(loc_index, "amogus_bogus_file");
	REQUIRE( test == "html/second_dir"); //Check loc 1 for file it contains

	test = s.getRootForFile(0, "amogus_bogus_file");
	REQUIRE( test == "");  //Check loc 0 for file in loc 1
